...
### The device ontology — why the scope criteria are also OWL
`ontology/homeiot.ttl` encodes the five definitional criteria as an OWL equivalence axiom over a
facet vocabulary, with every category carrying an explicit value for all five. It exists for
...
2. **The tail problem needs a coarser tier.** Six leaf categories rest on ≤5 confirmed CVEs yet
   ...
   **supervisor-agreed folding categories** are the fix; `--group family` on
   `cwe888_analysis.py`/`cvss_analysis.py` reports at that level. Folding is a **view** over
   existing judgments — it never re-labels a CVE, and per-category numbers are always reported
   alongside, never instead.
3. **It generates `categories.csv` byte-identically**, so the formal model and the file every
   reviewer's scope note actually comes from cannot drift apart.

**Circularity boundary (do not cross).** The ontology holds *classes and scope notes*, never
judgments. `scope_note` text is **moved** into the ontology verbatim, never synthesized, so no
reviewer sees information the prose note didn't already carry and no measured precision changes.
Deriving judgments from facets would make `term_precision.csv` self-confirming — the same hazard
`PLAN_deterministic_preclassifier.md` guards against.
...
### Snapshot vintage — pinned is not frozen

Pinning means *both search methods see the same snapshot*, so a vendor/keyword gap reflects search
terms rather than data freshness. It does **not** mean the snapshot never moves: re-downloading and
re-running both searches preserves the invariant at a new date. The mechanical trap is that
`download_nvd.py` is built to **resume** — it skips CVE IDs already present — so re-pointing it at
a populated `nvd_all.csv` appends only new CVEs and leaves every existing row at its old vintage.
Always download to a fresh path, run `snapshot_churn.py` **before** cutting over (the old vintage
is irreproducible — NVD serves current state only), then swap.

**Measured, 2026-06-25 → 2026-08-05 (`snapshot_churn.md`):** over the 1,676 confirmed-Yes CVEs,
NVD changed almost nothing — 0 base-score changes, 0 CWE backfills on the 118 CWE-less rows, 3 CPE
backfills on the 196 CPE-less ones, 0 v2.0 records re-scored under v3.x, 0 newly REJECTED. All
three real changes are 2026 CVEs NVD was still analysing for the *first* time. So **reanalysis
churn on already-analysed records is effectively nil**, and the two standing gaps — 7.0% of the
confirmed set with no usable CWE, 11.5% with no CPE — are **not** backlog artefacts that a later
refresh will clear. Treat them as limitations to report, and don't justify a re-download by them.
The refresh that *was* worth it fixed the parser, not the data: `cvssMetricV40` (54 CVEs that had
silently been `Unscored`) and `vector_string` (100% coverage, which is what unblocks RQ3).
...
└── data/                            # All datasets, grouped by search method
    ├── ontology/                     # GENERATED by ontology_build.py — families.csv (leaf→fold map) + homeiot-kg.ttl (instance KG)
    ├── categories.csv                # GENERATED from the ontology — the 24 frozen analysis categories: slug, label, scope_note
    ├── nvd-snapshot/                 # Fixed offline NVD dataset (one snapshot, reproducible/citeable)
    │                                 #   + snapshot_churn.{csv,md} — what NVD changed vs the prior vintage
    │                                 #   + the superseded vintage, kept: nvd_all_<yyyy-mm>.csv (gitignored)
