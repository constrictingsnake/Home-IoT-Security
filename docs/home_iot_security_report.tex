...
OWL~2~RL reasoner over the criterion facets reproduces all 27 in-scope and out-of-scope
...
smart-home vocabularies quantifies how far the scope departs from existing practice. Of
the 24 categories, only 9 (38\%) have an exactly corresponding class in SAREF (core,
4BLDG, 4ENER, 4WEAR) or W3C SSN/SOSA; the remaining 15 (62\%) can be placed only under a
generic superclass such as \texttt{Device} or \texttt{Sensor}. The gap is not evenly
distributed: it falls almost entirely on the consumer security and monitoring tier ---
cameras, doorbells, baby monitors, alarms, door locks, hubs --- which carries 91.5\% of
our confirmed CVEs. The categories the standard vocabularies model precisely are largely
the ones with the fewest disclosed vulnerabilities. This supports the choice to define
categories from consumer product boundaries rather than adopt an existing vocabulary,
but it is equally a limitation on comparability: our categories cannot be mapped
one-to-one onto studies built on those vocabularies.
