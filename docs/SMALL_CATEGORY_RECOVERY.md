...
Status: **findings + next steps, not yet implemented.***
...
1. **Engine fix (F1)** — in `cve_search.py` `filter_by_keywords` whole-word mode, make a
   space in a term match `[ _]`:
   `re.escape(kw)` → `re.escape(kw).replace(r"\ ", "[ _]")`.
   Then regenerate both searches + downstream (README stage commands). ~180 new rows
   arrive across categories; refresh invariant keeps settled judgments.
